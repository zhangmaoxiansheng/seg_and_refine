function [x_f,y_f,flag] = edge_around(x,y,depth_map,edge_depth,thres,min_thres,upper,lower,mean)
    flag = 0;
    x_f = x;
    y_f = y;
    up = x - thres;
    ...
    for i = 1 : thres
        ...
        if(edge_r(i) == 1 && vec_r(i) < upper && vec_r(i) > lower && (abs(vec_r(i)-mean)<mean_disp))
        ...
        if(edge_l(end+1-i) == 1 && vec_l(end+1-i) < upper && vec_l(end+1-i) > lower && abs(vec_l(end+1-i)-mean)<mean_disp)
        ...
        if(edge_d(i) == 1 && vec_d(i) < upper && vec_d(i) > lower && abs(vec_d(i)-mean)<mean_disp)
        ...
        if(edge_u(end+1-i) == 1 && vec_u(end+1-i) < upper && vec_u(end+1-i) > lower && abs(vec_u(end+1-i)-mean)<mean_disp)
    ...
    if(direction ~= 0 && edge_pix > min_thres)%depth edge can be found and the pixel is reasonable
